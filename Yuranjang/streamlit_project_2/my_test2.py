...
st.caption(':blue[다른 사람의 지원정보 미리보기]:sunglasses:')
scores = pd.read_csv(
...
# 예측
submitted = st.button('예측 결과 보기')

# preval1 = model.predict([[100, 1.50, 1]])
# print(f'당신의 합격 확률은 {preval1*100}% 입니다.')

if submitted:
    preval = model.predict([[toeic, number, option]])
    st.text(f'당신의 합격 확률은 {preval*100}% 입니다.')
