...
board2 = Blueprint('board2',__name__)
...
@board2.route("/image_preprocess", methods=['POST'])
def preprocessing():
...
def classification():
    if request.method == 'POST' or request.method == 'GET':
        ...
        predict = predicts
        return redirect(url_for('board2.result', pre = predict, idx=max_idx, percent=predict_max_percent))

@board2.route("/result/<pre>&<idx>&<percent>",methods=["POST","GET"])
